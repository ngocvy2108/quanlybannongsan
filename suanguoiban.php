...
if (!isset($_GET['id'])) {
    header("Location: quanlibanhang.php");
    exit();
}

$id = $_GET['id'];
$sql = "SELECT * FROM nguoiban WHERE IdNguoiBan='$id'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $ngay = date('Y-m-d', strtotime($_POST['NgayThamGia']));

    $update_sql = "UPDATE nguoiban 
                   SET TenNguoiBan='$ten', DiaChi='$diachi', SDT='$sdt', Email='$email', 
                       MoTaGianHang='$mota', NgayThamGia='$ngay'
                   WHERE IdNguoiBan='$id'";
    if ($conn->query($update_sql) === TRUE) {
        header("Location: suanguoiban.php?id=" . $id . "&update=success");
        exit();
    } else {
        header("Location: suanguoiban.php?id=" . $id . "&update=error&message=" . urlencode($conn->error));
        exit();
    }
}
?>
...
<head>
    ...
    <title>Cập nhật thông tin người bán</title>
    <style>
        body {
...
.nut-haichucnang input[type="button"] {
    background-color: #2e7d32;
    color: white;
}
.nut-haichucnang input[type="button"]:hover {
    background-color: #1b5e20;
}
...
.toast.success {
    ...
    <script>
        function xacNhanCapNhat() {
            return confirm("Bạn có chắc chắn muốn cập nhật thông tin người bán?");
        }
    </script>
</head>
<body>
...
<div class="khung">
    <div class="tieude">CẬP NHẬT THÔNG TIN NGƯỜI BÁN</div>
    <form id="form-update-seller" method="post">
        <div class="group">
            <label>ID Người Bán:</label>
            <input type="text" value="<?php echo $row['IdNguoiBan']; ?>" disabled>
        </div>
        <div class="group">
            <label>Tên Người Bán:</label>
            <input type="text" name="TenNguoiBan" value="<?php echo $row['TenNguoiBan']; ?>" pattern="[A-Za-zÀ-ỹ\s]+" title="Chỉ được nhập chữ cái và khoảng trắng" required>
        ...
        <div class="group">
            <label>Email:</label>
            <input type="email" name="Email" value="<?php echo $row['Email']; ?>" pattern="[a-z0-9._%+-]+@gmail\.com$" title="Email phải là địa chỉ Gmail hợp lệ" required>
            <span class="error-message" id="emailError"></span>
        </div>
        <div class="group">
            <label>Mô tả gian hàng:</label>
            <input type="text" name="MoTaGianHang" value="<?php echo $row['MoTaGianHang']; ?>">
        </div>
        <div class="group">
            <label>Ngày tham gia:</label>
            <input type="date" name="NgayThamGia" value="<?php echo date('Y-m-d', strtotime($row['NgayThamGia'])); ?>">
        </div>
        <div class="group">
...
function validateForm() {
    let isValid = true;
    const tenInput = document.querySelector('input[name="TenNguoiBan"]');
    ...
    // Validate Email
    if (!emailInput.value.match(/[a-z0-9._%+-]+@gmail\.com$/)) {
        document.getElementById('emailError').textContent = 'Email phải là địa chỉ Gmail hợp lệ';
...
<div id="modal-xac-nhan-update" class="modal-xoa" style="display:none;z-index:99999;">
  <div class="modal-xoa-content">
    <h3 style="color:#257a2a;">Xác nhận cập nhật thông tin</h3>
    <p>Bạn có chắc chắn muốn cập nhật thông tin người bán này không?</p>
...
function showToast(msg, success = true) {
    ...
    if (success) {
        setTimeout(() => {
            window.location.href = 'quanlibanhang.php';
        }, 2000);
    } else {
        setTimeout(() => {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (updateStatus === 'success') {
        showToast('✅ Cập nhật thông tin người bán thành công!', true);
