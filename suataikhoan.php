...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $update_sql = "UPDATE taikhoan 
                   SET TenDangNhap='$ten', MatKhau='$matkhau'
                   WHERE IdTaiKhoan='$id'";
    if ($conn->query($update_sql) === TRUE) {
        header("Location: suataikhoan.php?id=" . $id . "&update=success");
        exit();
    } else {
        header("Location: suataikhoan.php?id=" . $id . "&update=error&message=" . urlencode($conn->error));
        exit();
    }
}
?>
...
.toast.success {
    ...
    <script>
        function xacNhanCapNhat() {
            return confirm("Bạn có chắc chắn muốn cập nhật tài khoản?");
        }
    </script>
</head>
<body>
...
<div class="khung">
    <div class="tieude">CẬP NHẬT TÀI KHOẢN</div>
    <form id="form-update-account" method="post">
        <div class="group">
            <label>ID Tài khoản:</label>
            <input type="text" value="<?php echo $row['IdTaiKhoan']; ?>" disabled>
        </div>
        <div class="group">
            <label>Tên đăng nhập:</label>
            <input type="text" name="TenDangNhap" value="<?php echo $row['TenDangNhap']; ?>" pattern=".{3,}" title="Tên đăng nhập phải có ít nhất 3 ký tự" required>
            <span class="error-message" id="usernameError"></span>
        </div>
        <div class="group">
            <label>Mật khẩu:</label>
            <input type="password" name="MatKhau" value="<?php echo $row['MatKhau']; ?>" pattern="^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$" title="Mật khẩu phải có ít nhất 8 ký tự, bao gồm chữ hoa, chữ thường, số và ký tự đặc biệt" required>
            <span class="error-message" id="passwordError"></span>
            <div id="password-requirements" style="margin-top: 10px; font-size: 14px;">
                <div class="requirement" id="length">✗ Ít nhất 8 ký tự</div>
                <div class="requirement" id="uppercase">✗ Ít nhất 1 chữ hoa</div>
                <div class="requirement" id="lowercase">✗ Ít nhất 1 chữ thường</div>
                <div class="requirement" id="number">✗ Ít nhất 1 số</div>
                <div class="requirement" id="special">✗ Ít nhất 1 ký tự đặc biệt</div>
            </div>
        </div>
...
function validateForm() {
    ...
    if (usernameInput.value.length < 3) {
        ...
        usernameInput.style.borderColor = '#e53935';
        isValid = false;
    } else {
        document.getElementById('usernameError').style.display = 'none';
        usernameInput.style.borderColor = '#2e7d32';
    }
    ...
    if (!isValid) {
        document.getElementById('passwordError').textContent = 'Mật khẩu không đáp ứng các yêu cầu';
...
document.addEventListener('DOMContentLoaded', function() {
    const form = document.getElementById('form-update-account');
    ...
    // Xử lý hiển thị thông báo từ URL parameters
    const urlParams = new URLSearchParams(window.location.search);
    const updateStatus = urlParams.get('update');
    const errorMessage = urlParams.get('message');
    
    if (updateStatus === 'success') {
        showToast('✅ Cập nhật thông tin tài khoản thành công!', true);
...
<div id="modal-xac-nhan-update" class="modal-xoa" style="display:none;z-index:99999;">
  <div class="modal-xoa-content">
    ...
    <p>Bạn có chắc chắn muốn cập nhật thông tin tài khoản này không?</p>
...
function showToast(msg, success = true) {
    ...
    if (success) {
        // Nếu là thông báo thành công, chờ 2 giây rồi mới chuyển hướng
        setTimeout(() => {
            window.location.href = 'quanlitaikhoan.php';
        }, 2000);
    } else {
        // Nếu là thông báo lỗi, chỉ ẩn thông báo sau 2 giây
        setTimeout(() => {
